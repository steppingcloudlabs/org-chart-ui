...
console.log("node==",node)
  ...
  this.$refs.jobDrawerStack.openDrawer(jobProfileData);
    ...
    getPayGrade(orgChartData) {
      ...
      for (var i = 0; i < orgChartData?.length; i++) {
        ...
        // }
        const deptValue =
          orgChartData[i].userDepartmentName || orgChartData[i].department;
        ...
        if (
          businessUnitValue &&
          !this.businessunit.includes(businessUnitValue)
        ) {
  ...
  if (
    ...
    getData() {
      if (this.userData) {
        console.log("This.userdtaaaaaaaaaaa=",this.userData)
  ...
  this.nodes.map(n => ({
    ...
    img_binding(sender, node) {
      ...
      if (this.imgRequire == true) {
        ...
        var field =
          '<clipPath id="ulaImg"><circle cx="90" cy="60" r="40" stroke="white" stroke-width="5"></circle></clipPath><image preserveAspectRatio="xMidYMid slice" clip-path="url(#ulaImg)" xlink:href="' +
          (data.img && data.img.indexOf("https") > -1
            ? data.img
            : `data:image/jpg;base64,${data.img}`) +
          '" x="50" y="20"  width="80" height="80"></image>';
        //console.log(field)
        return field;
      } else {
    ...
    mytree: function (domEl, x, test = false) {
      ...
      OrgChart.templates.myTemplate.field_1 =
        '<text text-overflow="multiline" style="font-size: 14px;" fill="white" x="90" y="190" text-anchor="middle">{val}</text>';
      OrgChart.templates.myTemplate.field_4 =
        '<text  text-overflow="multiline" style="font-size: 14px;" fill="white" x="60" y="210" text-anchor="middle">{val}</text>';
      OrgChart.templates.myTemplate.field_5 =
        '<text text-overflow="multiline" style="font-size: 14px;" fill="white" x="120" y="210" text-anchor="middle">({val})</text>';
      ...
      OrgChart.templates.myTemplate.field_10 = function (
        node,
        data,
        template,
        config,
...
//   }
      ...
      this.chart.onRedraw(() => {
        ...
        if (this.selectedView) {
          ...
          if (!legenddEl) {
            ...
            legent1.innerHTML =
              "<p style = 'font-size:10px;font-weight: bold;text-align: center;margin-bottom:1px'>Pay Grade Count </p>" +
              this.str;
      ...
      this.chart.filterUI.on("add-item", function (sender, args) {
        ...
        args.html = `<div class="filter-item">
                 <input ${dataAllAttr} type="checkbox" id="${
          args.value
        }" name="${args.value}" ${args.checked ? "checked" : ""}>
      ...
      this.chart.filterUI.on("show-items", function (sender, args) {
        var filterItemElements =
          sender.element.querySelectorAll(".filter-item");
